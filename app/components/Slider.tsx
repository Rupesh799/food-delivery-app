...
const Slider = () => {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-6rem)] md:h-[calc(100vh-4rem)] lg:flex-row ">
            {/*text*/}
           <div className="flex-1 font-bold text-2xl p-6 flex flex-col gap-8 items-center justify-center ">
               <h1 className="text-center text-5xl uppercase">{sliderData[currentSlide].title}</h1>
               <button className="px-4 py-3 bg-primary-primary2 rounded ">Order Now</button>
           </div>
            {/*image*/}
            <div className='w-full flex-1 relative l'>
               <Image src={sliderData[currentSlide].image} alt='' fill className="object-cover"/>
            </div>
        </div>
    );
};
